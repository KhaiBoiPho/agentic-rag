...
import { Book, Chart, Chat, Folder, Gear, Logout, Note, Plus } from "./Icons";
    ...
    <aside className="side">
      ...
      <button className="newbtn" onClick={newChat}>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
          <Plus width={16} height={16} /> {t.common.newChat}
        </span>
        <kbd>⌘K</kbd>
      </button>
      ...
      <nav className="nav">
        {NAV.map(({ href, label, Icon }) => {
          ...
          return (
            <Link key={href} href={href} className={on ? "on" : ""} onClick={onNavigate}>
              <Icon />
              {label}
            </Link>
          );
