% !TeX program = xelatex
% -----------------------------------------------------------------------------
% This document must be compiled with XeLaTeX (or LuaLaTeX), not pdfLaTeX.
% The line above is a "magic comment" that Overleaf, TeXstudio, VS Code's
% LaTeX Workshop, and most other editors read automatically to select the
% correct compiler. If your editor does not honour it, set the compiler to
% XeLaTeX manually (in Overleaf: Menu -> Compiler -> XeLaTeX).
% XeLaTeX is used because it supports Vietnamese Unicode characters (diacritics
% such as ễ, ả, ố, ...) natively through fontspec, with no font-encoding
...
\usepackage{fontspec}
% TeX Gyre Termes is a Times-like font with broad Unicode coverage, including
% Vietnamese diacritics, and ships with every standard TeX Live/Overleaf
% installation, so no extra font files need to be uploaded.
...
% Under XeLaTeX with a Unicode-capable font (set above), Vietnamese text needs
% no special encoding switch, so this simply passes the text through. The
% command is kept (rather than removed) so the rest of the document does not
% need to change.
