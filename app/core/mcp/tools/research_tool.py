...
RESEARCH_TOOL = Tool(
    name="deep_research",
    description="Run a deep research workflow (6-node LangGraph) and return a comprehensive answer.",
    inputSchema={
        "type": "object",
        "properties": {
...
async def handle_deep_research(args: dict) -> list[TextContent]:
    ...
    async for event in graph.stream(
        query=args["query"],
        user_id="mcp-tool",
        config={"max_iterations": args.get("max_iterations", 2), "max_search_results": 8, "quality_threshold": 0.7},
    ):
